...
const fs = require('fs');
const updater = require('./utils/autoUpdater');
...
function createTrayMenu() {
  ...
  return Menu.buildFromTemplate([
    ...
    {
      label: '⬆️ Kiểm tra cập nhật (v' + updater.getCurrentVersion() + ')',
      click: async () => {
        await updater.checkAndPromptUpdate(false);
      }
    },
    { type: 'separator' },
    {
...
app.whenReady().then(async () => {
  ...
  // Auto-check for updates silently after 30 seconds
  setTimeout(async () => {
    try {
      const result = await updater.checkForUpdatesSilent();
      if (result.hasUpdate) {
        addLog(`Có phiên bản mới: v${result.latestVersion} (hiện tại: v${result.currentVersion})`, 'info');
      }
    } catch (e) {
